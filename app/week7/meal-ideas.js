...
const MealIdeas = ({ ingredients }) => {
    ...
    return (
        <main className="flex flex-col items-center">
            <h1 className="text-3xl text-violet-800 font-bold mb-4 text-center">
                Meal Ideas for {ingredients}
            </h1>
            {meals.length === 0 ?
                <p className="text-violet-800 text-xl">No meal ideas found for {ingredients}</p> :
                <ul className="space-y-4">
                    {meals.map((meal) => (
                        <li key={meal.idMeal} className="flex flex-col items-center bg-violet-300 rounded-lg p-4 shadow-md" onClick={() => handleMealClick(meal.idMeal)}>
